...
class Bot(commands.Bot):
    def __init__(self):
        super().__init__(token=ACCESS_TOKEN, prefix='!', initial_channels=['makiaato_'])

    async def event_ready(self):
        # Bot says 'None' first, when no routine is set
        print(f'Logged in as | {self.nick}')
        print('Bot is ready!')
        await self.get_channel('makiaato_').send('Translation-Bot is ready! SeriousSloth')
    ...
    async def is_token_still_valid():
        if not check_access_token_validity():    
            refresh_access_token()
    is_token_still_valid.start()
...
def translate(source_text):
    source_text_cleaned = re.sub(r'[^\u3000-\u303f\u3040-\u309f\u30a0-\u30ff\uff00-\uff9f\u4e00-\u9faf\u3400-\u4dbf]', '', source_text) # unicode numbers should cover every japanese character
